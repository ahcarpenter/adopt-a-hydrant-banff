...
protected
  ...
  def set_locale
    ...
    if request.env['HTTP_USER_AGENT'].include? 'Mobi'
      session[:url_for_oauth_code] = @@oauth.url_for_oauth_code :permissions=>'publish_stream', :permissions=>'email', :callback_url=>'http://adopt-a-hydrant-banff.herokuapp.com/', :display=>'touch'
    else
      session[:url_for_oauth_code] = @@oauth.url_for_oauth_code :permissions=>'publish_stream', :permissions=>'email', :callback_url=>'http://adopt-a-hydrant-banff.herokuapp.com/'
    end
